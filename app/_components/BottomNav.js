...
const Btn = styled.button`
  display: grid; place-items: center; gap: 6px; background: none; border: 0;
  color: ${(p)=> p.$active ? '#fff' : '#aaa'};
  font-size: 12px;
`;
const Icon = styled.span`
  width: 24px; height: 24px; display: inline-block;
  background-image: url(${(p)=>p.$src || 'none'}); background-size: contain;
...
export default function BottomNav({ current = "clip", onChange }) {
  ...
  return (
    <NavBar>
      {items.map((it)=> (
        <Btn key={it.key} $active={current===it.key} onClick={()=>{ onChange?.(it.key); if(it.to) router.push(it.to); }}>
