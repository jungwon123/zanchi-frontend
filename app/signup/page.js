...
export default function SignupPage() {
  ...
  React.useEffect(() => {
    ...
    if (!vv) return;
    const update = () =>
      setKbBottom(Math.max(0, window.innerHeight - vv.height - vv.offsetTop));
  ...
  React.useEffect(() => {
    if (step === 1 && stage1Ok) setStep(2);
  }, [step, stage1Ok]);
  React.useEffect(() => {
    if (step === 2 && stage2Ok) setStep(3);
  }, [step, stage2Ok]);
      ...
      <Section>
        <Step $active={step === 1} aria-hidden={step !== 1}>
          <Field>
            ...
            />
            {id && (
              <ClearBtn $right="10px" onClick={() => setId("")}>
                ×
              </ClearBtn>
            )}
          </Field>
          ...
          <Field>
            <Input
              type={pwVis1 ? "text" : "password"}
              placeholder="비밀번호"
              value={pw}
              onChange={(e) => setPw(e.target.value)}
            />
            {pw && (
              <ClearBtn $right="44px" onClick={() => setPw("")}>
                ×
              </ClearBtn>
            )}
            <EyeBtn onClick={() => setPwVis1((v) => !v)}>
              {pwVis1 ? "🙈" : "👁️"}
            </EyeBtn>
          </Field>
          ...
          <Field>
            <Input
              type={pwVis2 ? "text" : "password"}
              placeholder="비밀번호 확인"
              value={pw2}
              onChange={(e) => setPw2(e.target.value)}
            />
            {pw2 && (
              <ClearBtn $right="44px" onClick={() => setPw2("")}>
                ×
              </ClearBtn>
            )}
            <EyeBtn onClick={() => setPwVis2((v) => !v)}>
              {pwVis2 ? "🙈" : "👁️"}
            </EyeBtn>
          </Field>
          <Helper>
            {pw2 && !pwMatch ? "비밀번호가 일치하지 않습니다" : ""}
          </Helper>
        </Step>
        ...
        <Step $active={step === 2} aria-hidden={step !== 2}>
          <Field>
            ...
            />
            {phone && (
              <ClearBtn $right="44px" onClick={() => setPhone("")}>
                ×
              </ClearBtn>
            )}
          ...
          <Helper>
            {phone && !phoneValid ? "올바르지 않은 번호입니다" : ""}
          </Helper>
          ...
          <Field>
            ...
            />
            {code && (
              <ClearBtn $right="10px" onClick={() => setCode("")}>
                ×
              </ClearBtn>
            )}
          </Field>
          <Helper>
            {code && codeSent && code !== expectedCode
              ? "일치하지 않습니다"
              : ""}
          </Helper>
        </Step>
        ...
        <Step $active={step === 3} aria-hidden={step !== 3}>
          <Field>
            ...
            />
            {name && (
              <ClearBtn $right="10px" onClick={() => setName("")}>
                ×
              </ClearBtn>
            )}
          </Field>
