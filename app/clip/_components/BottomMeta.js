...
export default function BottomMeta({
  profile = { name: "사용자", following: false, id: null },
  title = "",
  desc = "",
  isMine = false,
  clipId = null,
  ...
  return (
    <MetaWrap>
      <Avatar
        ...
        style={{
          cursor: profile.id != null ? "pointer" : "default",
          backgroundImage: `url(${
            profile?.avatarUrl
              ? toAbsoluteUrl(profile.avatarUrl)
              : profile?.uploaderAvatarUrl
              ? toAbsoluteUrl(profile.uploaderAvatarUrl)
              : "/icon/default.png"
          })`,
      ...
      <MetaTextWrap>
        <MetaHeaderRow>
          ...
          >
            <strong
              style={{ cursor: profile.id != null ? "pointer" : "default" }}
            >
              {profile.name}
            </strong>
            {!isMine && (
              <FollowBadge
                onClick={async (e) => {
                  ...
                  try {
                    if (following) {
                      const res = await unfollow(profile.id);
                      setFollowing(Boolean(res?.following));
                    } else {
                      const res = await toggleFollow(profile.id);
                      setFollowing(Boolean(res?.following ?? true));
                    }
                  } catch {}
                }}
