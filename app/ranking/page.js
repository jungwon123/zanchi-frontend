...
export default function RankingPage() {
  ...
  const list = React.useMemo(
    () => (data?.items || []).slice().sort((a, b) => b.score - a.score),
    [data?.items]
  );
  ...
  confirmDay.setDate(confirmDay.getDate() + 12); // D-12 in example
  const dday = Math.max(
    0,
    Math.ceil((confirmDay - today) / (1000 * 60 * 60 * 24))
  );
  ...
  React.useEffect(() => {
    const dayDiff = Math.floor(
      (confirmDay.setHours(0, 0, 0, 0) - new Date().setHours(0, 0, 0, 0)) /
        (1000 * 60 * 60 * 24)
    );
  ...
  const openClip = (clipId, uploaderId) =>
    router.push(`/clip/view?userId=${uploaderId}&clipId=${clipId}`);
          ...
          return order.map((u, idx) => {
            ...
            return (
              <PCard
                key={u.id}
                $scale={idx === 1 ? 1 : 0.8}
                onClick={() => openClip(u.clipId, u.uploaderId)}
              >
                <Crown
                  src={crowns[idx]}
                  alt="crown"
                  $w={widths[idx]}
                  $top={idx === 1 ? -50 : undefined}
                />
                <Media
                  as="div"
                ...
                <AvatarRow>
                  <AvatarImg
                    src={u.avatarUrl || "/icon/default.png"}
                    alt="profile"
                  />
                </AvatarRow>
                <Name>{u.name}</Name>
      ...
      <List>
        {others.map((u, i) => (
          <Row key={u.id} onClick={() => openClip(u.clipId, u.uploaderId)}>
            <RankNum style={{ color: "#040404" }}>{i + 4}</RankNum>
