...
export default function PaymentPage() {
  ...
  return (
    <Wrap>
      ...
      <PayMethods>
        {[
          ...
          { key: "toss", label: "Toss", src: "/images/payment/toss.png" },
          {
            key: "paybook",
            label: "Paybook",
            src: "/images/payment/paybook.png",
          },
        ].map((m) => (
          <PayBtn
            key={m.key}
      ...
      <PointBox>
        ...
        <PointMeta>
          <span style={{ paddingLeft: 4 }}>
            보유 포인트: {available.toLocaleString()}P
          </span>
        </PointMeta>
      </PointBox>
      ...
      <div
        style={{ color: "#888", fontSize: 13, lineHeight: 1.6, marginTop: 12 }}
      >
        환불 규정 안내
        <br />
        - 2~3일 전 취소 시 전액 환불
      ...
      <BottomBar>
        <PrimaryButton
          disabled={!method}
          onClick={() => {
            // 결제 성공 후 결제내역으로 이동 (완료 버튼 노출)
            router.push("/payment/history?from=success");
          }}
        >
