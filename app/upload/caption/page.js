...
export default function CaptionEditPage() {
  return (
    <React.Suspense
      fallback={<div style={{ padding: 16, color: "#999" }}>로딩중…</div>}
    >
      <CaptionEditInner />
    </React.Suspense>
  );
...
function CaptionEditInner() {
  ...
  const { data: me } = useQuery({
    queryKey: ["me-summary-for-caption"],
  ...
  const { data: followingData } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        return list
          .map((m) => ({ loginId: m?.loginId, avatarUrl: m?.avatarUrl }))
          .filter((m) => !!m.loginId);
      } catch {
        return [];
      }
  ...
  const data = useMemo(() => {
    if (mode === "@") {
      ...
      return raws
        .filter((m) =>
          String(m.loginId).toLowerCase().includes(String(key).toLowerCase())
        )
        .map((m) => ({
          type: "user",
          value: m.loginId,
  ...
  useEffect(() => {
    ...
    if (cap != null) {
      try {
        setCaption(decodeURIComponent(cap));
      } catch {
        setCaption(cap);
      }
    }
      ...
      <TopBar>
        ...
        <Confirm
          onClick={async () => {
            ...
            if (clipId) {
              try {
                await updateClipCaption(clipId, { caption });
              } catch {}
            }
            router.back();
          }}
