...
export default function UploadPage() {
  ...
  const { data: me } = useQuery({
    queryKey: ["me-summary-for-upload"],
    queryFn: getMySummary,
    staleTime: 60_000,
  });
  const keyForQuery = (keyword || "").startsWith("@")
    ? (keyword || "").slice(1)
    : "";
  const { data: followingData } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await getFollowing(me.id, {
          page: 0,
          size: 50,
          q: keyForQuery,
        });
        const list = Array.isArray(res?.content)
          ...
          ? res
          : [];
        return list.map((m) => m?.loginId).filter(Boolean);
      } catch {
        return [];
      }
    },
  ...
  const mentionData = useMemo(() => {
    ...
    if (mode === "#") {
      return hashtags
        .filter((h) => h.startsWith(keyword.replace("#", "")))
        .map((h) => ({ type: "hash", value: h }));
    }
    return [];
  ...
  return (
    <Container>
      ...
      <CaptionWrap>
        <CaptionLabel style={{ display: "none" }}>캡션</CaptionLabel>
        {!caption?.trim() ? (
          <button
            style={{
              border: "0",
            ...
            onClick={() => router.push("/upload/caption")}
          >
            캡션 추가…
          </button>
        ) : (
      ...
      <AgreeRow>
        <div style={{ fontSize: 12 }}>
          잔치픽 순위로 상위 10명에게 공연 기회가 주어지며, 이후 본인에게 참여
          의사가 달려 있음을 고지합니다
        </div>
        <Checkbox
          type="checkbox"
